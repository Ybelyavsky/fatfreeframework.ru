...
Динамические Обработчики Маршрутов Вот еще один Ф3 сюрпризом:

$Ф3-&gt;маршруте\('получить /продукты/@действие','товары-&gt;@действии'\); Если ваше приложение получает запрос, скажем, на /products / itemize, F3 извлекает строку 'itemize' из URL-адреса и передает ее маркеру @action в обработчике маршрута. Затем F3 будет искать класс с именем Products и выполнять его метод itemize \(\).

Динамические обработчики маршрутов могут иметь различные формы:

// статический метод $Ф3-&gt;маршруте\('получить /общественных/@жанр','Main::@genre'\); // объектный режим $Ф3-&gt;маршруте\('получить /общественно - /@контроллер/@действие", "контроллер-&gt;@@действии'\); F3 вызывает ошибку HTTP 404 Not Found во время выполнения, если он не может передать управление классу или методу, связанному с текущим маршрутом, т. е. нет определенного класса и/или метода, соответствующего запрошенному маршруту.

AJAX и синхронные запросы Шаблоны маршрутизации могут содержать модификаторы, которые предписывают фреймворку основывать свое решение о маршрутизации на типе HTTP-запроса:
...
Маршрутизация в режиме CLI Веб-синтаксис Если вы хотите запустить определенный маршрут из инструмента командной строки, сценария оболочки, консоли или задания cron, вы можете эмулировать запрос HTTP GET следующей командой:
...
// синтаксис \#1 \(Проверка данных, смешивание коротких и длинных форм\) $выберет=\[ 'force' = &gt; $f3 - &gt;exists \('GET.f'\), / / - f 'limit' =&gt; abs \(\(int\)$f3-&gt;get \('GET.предел'\)\)?: 20, / / --limit=N \(по умолчанию: 20\) 'verbose' = &gt; $f3 - &gt;exists \('GET.v'\) \| / $f3 - &gt;exists \('GET.verbose'\), / / - v или --verbose \];

// синтаксис \#2 \(не проверка данных, только длинная форма\) $opts=$\_GET+\[ 'force'=&gt;0, / / --force 'limit'=&gt;20, / / --limit=N \(по умолчанию: 20\) 'verbose'=&gt;0, / / --verbose \]; Определение маршрутов Маршруты CLI определяются так же, как и веб-маршруты, с той лишь разницей, что они могут быть ограничены режимом CLI с помощью модификатора \[cli\]. Например:

\[пути сообщения\] GET /log/show \[cli\] = CLI\Log-&gt;показать Получить /журнал/очистить \[Кинк\] = Кинк\журнал-&gt;очистить GET /cache/clear \[cli\] = CLI\Cache-&gt;очистить GET /help \[cli\] = CLI\Help - &gt; индекс Сделать /помочь команде/@\[Кинк\] = командной строки справка-&gt;\команды@ Примечание: переменная Кинк говорит, если запрос приходит из командной строки или нет.
